...
wholesale_scored AS (
    SELECT
        *,
        CASE
            WHEN wholesale_stddev IS NULL
                 OR wholesale_stddev = 0
                 OR preceding_count < 7
            THEN NULL

            WHEN ABS(
                (wholesale_price - wholesale_baseline)
                / wholesale_stddev
            ) >= 10000
            THEN NULL
...
retail_scored AS (
    SELECT
        *,
        CASE
            WHEN retail_stddev IS NULL
                 OR retail_stddev = 0
                 OR preceding_count < 7
            THEN NULL

            WHEN ABS(
                (retail_price - retail_baseline)
                / retail_stddev
            ) >= 10000
            THEN NULL
...
SELECT
    ...
    CASE
        WHEN ws.wholesale_price IS NULL
             OR ws.wholesale_price <= 0
             OR rs.retail_price IS NULL
        THEN NULL

        WHEN ABS(
            (
                (rs.retail_price - ws.wholesale_price)
                / ws.wholesale_price
                * 100
            )
        ) >= 10000
    ...
    CASE
        WHEN (
            ws.wholesale_zscore IS NOT NULL
            AND ABS(ws.wholesale_zscore) > 2
        )
        OR (
            rs.retail_zscore IS NOT NULL
            AND ABS(rs.retail_zscore) > 2
        )
        THEN true
